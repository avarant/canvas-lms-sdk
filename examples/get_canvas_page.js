/**
 * Example: Get a specific Canvas page using the Canvas LMS SDK (JavaScript version)
 * 
...
const { Configuration, PagesApi } = require('../generated_typescript_sdk/dist');
...
async function getCanvasPage(courseId, pageUrl) {
...
async function listAllPagesInCourse(courseId) {
  // Configure the API client
  const configuration = new Configuration({
    basePath: CANVAS_HOST,
    headers: {
...
async function main() {
  ...
  if (args.length < 1) {
    console.log('Usage:');
    console.log('  List all pages:     node get_canvas_page.js <course_id>');
    console.log('  Get specific page:  node get_canvas_page.js <course_id> <page_url>');
    console.log('');
    console.log('Examples:');
    console.log('  node get_canvas_page.js 458030');
    console.log('  node get_canvas_page.js 458030 arc-agi-project-overview');
    process.exit(1);
  }
